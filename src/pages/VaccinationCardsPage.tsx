...
export const VaccinationCardsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const vaccinesData: Record<string, Vaccine[]> = {};
        await Promise.all(userPets.map(async (pet) => {
          const vaccines = await petService.getVaccines(pet.id);
          vaccinesData[pet.id] = vaccines;
        }));
        setVaccinesByPet(vaccinesData);
      } catch (error) {
  ...
  const handleDownload = (pet: Pet) => {
    ...
    if (btn) {
      ...
      setTimeout(() => {
        ...
        alert(`Carteira de vacinação de ${pet.name} baixada com sucesso! (Simulação)`);
      }, 1500);
    }
  };
  ...
  return (
    <Layout title="Carteira de Vacinação" headerType="default">
      <div className="flex-1 overflow-y-auto pb-24 bg-gray-50">
        <div className="max-w-md mx-auto px-4 py-6 space-y-6">
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900 uppercase tracking-tight">Carteira de Vacinação</h1>
              <p className="text-xs text-gray-500 font-bold uppercase tracking-widest">Controle de imunização</p>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-white rounded-2xl border border-gray-100 shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-sm font-medium"
            />
          </div>
          ...
          {loading ? (
                ...
                >
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Vacinas Aplicadas</h3>
                      <button 
                        onClick={() => navigate(`/pet/${pet.id}`, { state: { openVaccineModal: true } })}
                    ...
                    {vaccinesByPet[pet.id]?.length > 0 ? (
                      <div className="space-y-3">
                        {vaccinesByPet[pet.id].map((vaccine) => (
                          <div key={vaccine.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-0.5">
                                <span className="text-[10px] text-gray-500 font-bold flex items-center gap-1">
                                  <Calendar size={10} />
                                  {format(vaccine.applicationDate instanceof Date ? vaccine.applicationDate : vaccine.applicationDate.toDate(), 'dd/MM/yyyy')}
                                </span>
                                {vaccine.nextDose && (
                                  <span className="text-[10px] text-emerald-600 font-black uppercase tracking-tighter">
                                    Próxima: {format(vaccine.nextDose instanceof Date ? vaccine.nextDose : vaccine.nextDose.toDate(), 'dd/MM/yyyy')}
                                  </span>
                                )}
                              </div>
                            </div>
                      ...
                      <div className="text-center py-8 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                        <Syringe className="mx-auto text-gray-300 mb-2" size={32} />
                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Nenhuma vacina registrada</p>
                        <button 
                          onClick={() => navigate(`/pet/${pet.id}`, { state: { openVaccineModal: true } })}
                          className="mt-3 text-[10px] font-black text-emerald-600 uppercase tracking-widest underline"
                        >
                  ...
                  <div className="px-6 pb-6 grid grid-cols-2 gap-3">
                    <button 
                      onClick={() => navigate(`/pet/${pet.id}`, { state: { openVaccineModal: true } })}
                      className="flex items-center justify-center gap-2 py-3 bg-emerald-50 text-emerald-700 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-colors"
                    >
                      <Plus size={14} />
                      Inserir Vacina
                    </button>
            ...
            <div className="text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-gray-100">
              ...
              <button 
                ...
                className="bg-emerald-600 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-emerald-100"
              >
                Cadastrar Meu Pet
              </button>
            </div>
